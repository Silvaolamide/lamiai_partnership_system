...
<body class="min-h-screen bg-slate-50 text-slate-900">
    <main class="mx-auto max-w-3xl px-4 py-8 sm:py-12">
            ...
            <section class="mb-6 rounded-2xl border-2 border-emerald-200 bg-emerald-50 p-6 shadow-sm">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <a href="{{ url('/customer/dashboard') }}" class="mt-5 inline-flex items-center justify-center rounded-xl bg-emerald-600 px-5 py-3 font-black text-white shadow-sm transition hover:bg-emerald-700">
                            Go to My Dashboard →
                        </a>
                    </div>
                </div>
        ...
        @if(!session('bank_transfer_submitted'))
            <section class="mb-6 rounded-2xl border-2 border-violet-200 bg-white p-5 shadow-sm sm:p-6">
                ...
                <div class="mt-5 space-y-4">
                    <div class="flex gap-4 rounded-xl bg-violet-50 p-4">
                        ...
                        <div><p class="font-black">Make the payment</p><p class="mt-1 text-sm leading-6 text-slate-600">Transfer the <strong>exact amount</strong> shown below into the AIPM bank account.</p></div>
                    </div>
                    <div class="flex gap-4 rounded-xl bg-blue-50 p-4">
                        ...
                        <div><p class="font-black">Attach your proof</p><p class="mt-1 text-sm leading-6 text-slate-600">After the transfer, complete the form below and attach your bank transfer receipt or proof of payment.</p></div>
                    </div>
                    <div class="flex gap-4 rounded-xl bg-emerald-50 p-4">
                        ...
                        <div><p class="font-black">Submit the proof — this places your order</p><p class="mt-1 text-sm leading-6 text-slate-600">When everything is complete, click <strong>Submit Payment Proof</strong>. This is the action that creates your order and sends the transfer for admin verification.</p></div>
                    </div>
                </div>
            </section>
            ...
            <div class="space-y-5">
                <section class="rounded-2xl bg-slate-900 p-6 text-white shadow-sm sm:p-7">
                    <p class="text-sm font-bold text-slate-300">EXACT AMOUNT TO TRANSFER</p><p class="mt-1 text-4xl font-black tracking-tight">₦{{ number_format($product->price, 2) }}</p><p class="mt-3 text-sm text-slate-300">Product: {{ $product->name }}</p>
                </section>
        ...
        <section class="mt-6 rounded-2xl border border-blue-100 bg-blue-50 p-5">
            <h3 class="font-bold">Need help with this payment?</h3><p class="mt-1 text-sm text-slate-700">Contact support and quote the product name: <strong>{{ $product->name }}</strong>.</p>
            <div class="mt-3 flex flex-wrap gap-4 text-sm font-semibold">
                @if($paymentSettings->support_phone)<a href="tel:{{ $paymentSettings->support_phone }}">📞 {{ $paymentSettings->support_phone }}</a>@endif
                @if($paymentSettings->support_whatsapp)<a href="https://wa.me/{{ preg_replace('/\D/', '', $paymentSettings->support_whatsapp) }}">💬 WhatsApp</a>@endif
                @if($paymentSettings->support_email)<a href="mailto:{{ $paymentSettings->support_email }}">✉ {{ $paymentSettings->support_email }}</a>@endif
            </div>
        </section>
    </main>
    <script>
        document.getElementById('bank-transfer-form')?.addEventListener('submit', function () {
            ...
            if (button) { button.disabled = true; button.textContent = 'Submitting Payment Proof…'; button.classList.add('opacity-70', 'cursor-not-allowed'); }
        });
    </script>
</body>
</html>
