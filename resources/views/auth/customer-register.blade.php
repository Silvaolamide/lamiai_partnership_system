<x-guest-layout>
    <div class="mb-8">
        ...
        <h2 class="text-3xl font-extrabold tracking-tight text-[#171323]">Create your account.</h2>
        <p class="mt-2 text-sm leading-6 text-gray-500">Create an account to complete your purchase and access your orders.</p>
    </div>
    ...
    <form method="POST" action="{{ route('customer.register') }}" class="space-y-5">
        ...
        <div>
            ...
            <x-text-input id="email" class="brand-input block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3.5 text-sm" type="email" name="email" :value="old('email')" required autocomplete="username" placeholder="you@example.com" />
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <x-text-input id="password_confirmation" class="brand-input block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3.5 text-sm" type="password" name="password_confirmation" required autocomplete="new-password" placeholder="Repeat your password" />
                <x-input-error :messages="$errors->get('password_confirmation')" class="mt-2" />
            </div>
        </div>

        <p class="text-xs leading-5 text-gray-400">By creating an account, you agree to the platform's terms.</p>
        ...
        <button type="submit" class="brand-btn flex w-full items-center justify-center rounded-xl px-5 py-3.5 text-sm font-bold text-white shadow-lg shadow-violet-700/15">
            Create customer account <span class="ml-2 text-lg">→</span>
        </button>
    </form>
