<x-guest-layout>
    ...
    @if(session('status'))
        <div class="mb-5 rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{{ session('status') }}</div>
    @endif
    ...
    @if($order)
        <div class="mb-5 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">
            ...
            <p class="mt-1">Order {{ $order->order_number }} is ready to be added to your account.</p>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('customer.login') }}" class="space-y-5">
        ...
        <div>
            ...
            <x-text-input id="email" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3.5 text-sm" type="email" name="email" :value="old('email', $order?->customer_email)" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>
        <div>
            <x-input-label for="password" value="Password" class="mb-2 font-semibold text-gray-700" />
            <x-text-input id="password" class="block w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-3.5 text-sm" type="password" name="password" required autocomplete="current-password" />
        ...
        <button type="submit" class="flex w-full items-center justify-center rounded-xl bg-violet-700 px-5 py-3.5 text-sm font-bold text-white">Continue to my dashboard →</button>
    </form>
