...
@if(session('error'))<div class="mb-4 rounded-2xl bg-rose-50 border border-rose-200 p-4 text-rose-700">{{ session('error') }}</div>@endif
...
<td class="p-4 min-w-[420px]"><div class="space-y-3">
...
@if($membership->program?->products?->isNotEmpty())
<div class="mt-3 rounded-lg bg-white border p-3"><div class="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-2">Products in this program</div><div class="flex flex-wrap gap-2">@foreach($membership->program->products as $product)<span class="rounded-full bg-blue-50 text-blue-700 px-2.5 py-1 text-xs font-bold">{{ $product->name }}</span>@endforeach</div></div>
@else
<div class="mt-3 text-xs text-slate-400">No products are currently attached to this program.</div>
@endif
...
<td class="p-4">@php $recruiters = $partner->admin_memberships->map(fn ($m) => $m->parentPartner?->user?->name)->filter(fn ($name) => filled($name))->unique()->values(); @endphp
@if($recruiters->isEmpty())<span class="text-slate-500">Direct / none</span>@else<div class="flex flex-wrap gap-1">@foreach($recruiters as $recruiter)<span class="rounded-full bg-violet-50 text-violet-700 px-2.5 py-1 text-xs font-bold">{{ $recruiter }}</span>@endforeach</div>@endif</td>
