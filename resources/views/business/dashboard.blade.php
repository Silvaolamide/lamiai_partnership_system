...
<head>
    ...
    <title>{{ auth()->user()->business_name ?? 'Business' }} — LAMI AI</title>
...
<div class="min-h-screen lg:flex">
    <aside class="hidden w-72 shrink-0 border-r border-slate-200 bg-white lg:flex lg:flex-col">
        <div class="border-b border-slate-100 p-7">
            <a href="{{ route('business.dashboard') }}" class="text-xl font-black">LAMI <span class="text-violet-600">AI</span></a>
    ...
    <main class="min-w-0 flex-1">
        ...
        <div class="mx-auto max-w-7xl space-y-7 p-5 lg:p-10">
            ...
            <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <a href="{{ route('business.sales.index') }}" class="rounded-2xl bg-white p-6 shadow-sm hover:ring-2 hover:ring-violet-200"><p class="text-sm font-bold text-slate-500">Revenue</p><p class="mt-3 text-2xl font-black">₦{{ number_format($stats['revenue'],2) }}</p><p class="mt-1 text-xs text-slate-400">View sales →</p></a>
                <a href="{{ route('business.sales.index') }}" class="rounded-2xl bg-white p-6 shadow-sm hover:ring-2 hover:ring-violet-200"><p class="text-sm font-bold text-slate-500">Sales</p><p class="mt-3 text-2xl font-black">{{ number_format($stats['sales']) }}</p><p class="mt-1 text-xs text-slate-400">View orders →</p></a>
                <a href="{{ route('business.commissions.index') }}" class="rounded-2xl bg-white p-6 shadow-sm hover:ring-2 hover:ring-violet-200"><p class="text-sm font-bold text-slate-500">Commissions</p><p class="mt-3 text-2xl font-black">₦{{ number_format($stats['commission'],2) }}</p><p class="mt-1 text-xs text-slate-400">View commissions →</p></a>
                <a href="{{ route('business.affiliates.index') }}" class="rounded-2xl bg-white p-6 shadow-sm hover:ring-2 hover:ring-violet-200"><p class="text-sm font-bold text-slate-500">Affiliates</p><p class="mt-3 text-2xl font-black">{{ number_format($stats['affiliates']) }}</p><p class="mt-1 text-xs text-slate-400">View affiliates →</p></a>
            </section>
            ...
            <section class="grid gap-7 lg:grid-cols-2">
                ...
                <a href="{{ route('business.commissions.index') }}" class="rounded-2xl bg-white p-6 shadow-sm hover:ring-2 hover:ring-violet-200"><p class="text-xs font-black uppercase tracking-widest text-violet-600">Money</p><h2 class="mt-1 text-xl font-black">Commissions</h2><p class="mt-3 text-sm text-slate-500">Review partner earnings, payable commissions and payment status.</p><span class="mt-5 inline-block text-sm font-black text-violet-600">Manage commissions →</span></a>
            </section>
