...
<nav x-data="{ open: false, collapsed: false }" class="relative z-50">
    @if($isAdmin)
        <div class="min-h-screen bg-[#f6f7fb]">
            {{-- Desktop sidebar --}}
            <aside :class="collapsed ? 'w-[82px]' : 'w-[264px]'" class="fixed inset-y-0 left-0 hidden border-r border-slate-200/80 bg-white transition-all duration-300 lg:flex lg:flex-col">
                <div class="flex h-[78px] items-center border-b border-slate-100 px-5">
                    <a href="{{ route('admin') }}" class="flex min-w-0 items-center gap-3">
                        ...
                        <span x-show="!collapsed" x-transition class="truncate text-sm font-black tracking-tight text-slate-950">AI Powered <span class="text-violet-600">Marketing</span></span>
                    </a>
                </div>
                ...
                <div class="flex-1 overflow-y-auto px-3 py-5">
                    @foreach($adminSections as $section => $items)
                        <div class="mb-6">
                            <div x-show="!collapsed" class="mb-2 px-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">{{ $section }}</div>
                            <div class="space-y-1">
                                @foreach($items as $item)
                                    <a href="{{ route($item['route']) }}" class="group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold transition-all {{ request()->routeIs($item['route']) || ($item['route']==='admin.analytics.businesses' && request()->routeIs('admin.analytics.*')) ? 'bg-violet-50 text-violet-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-950' }}" title="{{ $item['label'] }}">
                                        @if(request()->routeIs($item['route']) || ($item['route']==='admin.analytics.businesses' && request()->routeIs('admin.analytics.*')))<span class="absolute left-0 h-6 w-1 rounded-r-full bg-violet-600"></span>@endif
                                        <span class="grid h-8 w-8 shrink-0 place-items-center rounded-lg {{ request()->routeIs($item['route']) ? 'bg-white text-violet-600 shadow-sm' : 'bg-slate-50 text-slate-400 group-hover:text-slate-700' }}">
                                            @include('layouts.icons', ['icon' => $item['icon']])
                                        </span>
                ...
                <div class="border-t border-slate-100 p-3">
                    <button @click="collapsed=!collapsed" class="mb-2 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-400 transition hover:bg-slate-50 hover:text-slate-700">
                        <span class="grid h-8 w-8 place-items-center rounded-lg bg-slate-50"><svg class="h-4 w-4 transition-transform" :class="collapsed ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m15 18-6-6 6-6"/></svg></span>
                    ...
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 rounded-xl bg-slate-50 p-3 transition hover:bg-violet-50">
                        ...
                        <span x-show="!collapsed" class="min-w-0"><b class="block truncate text-xs text-slate-900">{{ Auth::user()->name }}</b><small class="block truncate text-[10px] text-slate-400">Super Administrator</small></span>
                    </a>
                </div>
            </aside>

            {{-- Mobile drawer --}}
            <div x-show="open" x-transition.opacity class="fixed inset-0 z-[60] bg-slate-950/40 backdrop-blur-sm lg:hidden" @click="open=false"></div>
            <aside x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" class="fixed inset-y-0 left-0 z-[70] flex w-[290px] flex-col bg-white shadow-2xl lg:hidden">
                <div class="flex h-[78px] items-center justify-between border-b border-slate-100 px-5">
                    ...
                    <button @click="open=false" class="grid h-9 w-9 place-items-center rounded-xl bg-slate-50 text-slate-500"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 6 12 12M18 6 6 18"/></svg></button>
                </div>
                <div class="flex-1 overflow-y-auto px-3 py-5">
                    @foreach($adminSections as $section => $items)
            ...
            {{-- Admin top bar --}}
            <header class="fixed left-0 right-0 top-0 z-40 h-[78px] border-b border-slate-200/80 bg-white/90 backdrop-blur-xl lg:left-[264px]" :class="collapsed ? 'lg:left-[82px]' : 'lg:left-[264px]'">
                <div class="flex h-full items-center justify-between px-4 sm:px-6">
                    <div class="flex items-center gap-3">
                        <button @click="open=true" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-950 text-white lg:hidden"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16M4 12h16M4 18h16"/></svg></button>
    ...
    @else
        {{-- Lightweight shell for non-admin authenticated areas --}}
