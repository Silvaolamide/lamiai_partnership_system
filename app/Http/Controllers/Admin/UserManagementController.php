...
class UserManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->boolean('needs_attention')) {
            $query->where(function ($q) { $q->whereNull('email_verified_at')->orWhere(function ($q) { $q->where('registration_type', 'business')->where(function ($q) { $q->whereNull('business_super_admin_approved_at')->orWhereNull('business_name')->orWhereNull('business_phone'); }); }); });
        }
        ...
        $stats = [
            ...
            'partners' => User::query()->role('partner')->count(), 'customers' => User::query()->role('customer')->count(),
        ];
    ...
    public function show(User $user)
    {
        ...
        if (request()->boolean('partner_resolution')) {
            return app(\App\Http\Controllers\Admin\PartnerResolutionController::class)->show($user);
        }
        ...
        $data = [
            ...
            'paymentSubmissions' => PaymentSubmission::whereHas('order', fn ($q) => $q->where('customer_id', $user->id))->latest()->limit(20)->get(),
            'paymentDisputes' => PaymentDispute::where('customer_id', $user->id)->latest()->limit(20)->get(),
    ...
    public function edit(User $user) { $this->protectSuperAdmin($user); $user->load('roles'); return view('admin.users.edit', compact('user')); }
    ...
    public function update(Request $request, User $user): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'registration_type' => ['required', Rule::in(['customer', 'business'])], 'business_name' => ['nullable', 'string', 'max:255'],
            'business_website' => ['nullable', 'string', 'max:255'], 'business_industry' => ['nullable', 'string', 'max:255'], 'business_phone' => ['nullable', 'string', 'max:50'],
            'roles' => ['array'], 'roles.*' => [Rule::in(self::MANAGEABLE_ROLES)],
        ]);
    ...
    {
        $this->protectSuperAdmin($user); $activity = $this->activityFor($user);
    ...
    private function issuesFor(User $user): array
    {
        ...
        if ($user->hasRole('partner')) {
            $partnerIds = ProgramPartner::where('user_id', $user->id)->pluck('id');
            if ($partnerIds->isEmpty()) $issues[] = ['severity' => 'high', 'label' => 'Partner role has no program enrollment', 'action' => 'Review partner enrollment and attribution before changing commissions.'];
            if (Commission::whereIn('partner_id', $partnerIds)->whereIn('status', ['available', 'approved', 'payable'])->exists()) $issues[] = ['severity' => 'medium', 'label' => 'Partner has unresolved commission balance', 'action' => 'Review commission status and payout readiness.'];
        }
        return $issues;
    }
    ...
    {
        $checks = ['orders' => Order::where('customer_id', $user->id)->exists(), 'partner enrollments' => ProgramPartner::where('user_id', $user->id)->exists(), 'partner payouts' => Payout::where('partner_id', $user->id)->exists(), 'business payouts' => BusinessPayout::where('business_id', $user->id)->exists(), 'payment submissions' => PaymentSubmission::whereHas('order', fn ($q) => $q->where('customer_id', $user->id))->exists(), 'payment disputes' => PaymentDispute::where('customer_id', $user->id)->exists()];
