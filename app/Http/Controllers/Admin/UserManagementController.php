...
class UserManagementController extends Controller
{
        ...
        if ($request->boolean('needs_attention')) {
            $query->where(function ($q) {
                $q->whereNull('email_verified_at')
                    ->orWhere(function ($q) {
                        $q->where('registration_type', 'business')
                            ->where(function ($q) {
                                $q->whereNull('business_super_admin_approved_at')
                                    ->orWhereNull('business_name')
                                    ->orWhereNull('business_phone');
                            });
                    });
            });
        }
    ...
    public function update(Request $request, User $user): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'registration_type' => ['required', Rule::in(['customer', 'business'])],
            'business_name' => ['nullable', 'string', 'max:255'],
            'business_website' => ['nullable', 'string', 'max:255'],
            'business_industry' => ['nullable', 'string', 'max:255'],
            'business_phone' => ['nullable', 'string', 'max:50'],
            'roles' => ['array'],
            'roles.*' => [Rule::in(self::MANAGEABLE_ROLES)],
        ]);

        $user->forceFill(collect($validated)->except('roles')->all())->save();

        $roles = collect($validated['roles'] ?? [])->unique()->values()->all();
        if ($user->registration_type === 'business' && ! in_array('program_manager', $roles, true)) {
            $roles[] = 'program_manager';
        }
        if ($user->registration_type === 'customer' && ! in_array('customer', $roles, true)) {
            $roles[] = 'customer';
        }
    ...
    public function destroy(User $user): RedirectResponse
    {
        ...
        if ($activity['total'] > 0) {
            return back()->with('error', "This account cannot be permanently deleted because it has associated activity: {$activity['summary']}. Resolve, refund or archive that activity first.");
        }
    ...
    {
        abort_if($user->hasRole('super_admin'), 403, 'Super Admin accounts are protected from user-management operations.');
    }
    ...
    {
        $checks = [
            'orders' => Order::where('customer_id', $user->id)->exists(),
            'partner enrollments' => ProgramPartner::where('user_id', $user->id)->exists(),
            'partner payouts' => Payout::where('partner_id', $user->id)->exists(),
            'business payouts' => BusinessPayout::where('business_id', $user->id)->exists(),
            'payment submissions' => PaymentSubmission::whereHas('order', fn ($q) => $q->where('customer_id', $user->id))->exists(),
            'payment disputes' => PaymentDispute::where('customer_id', $user->id)->exists(),
        ];
