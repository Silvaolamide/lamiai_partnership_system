...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $stats = [
            ...
            'payable_commissions' => (float) Commission::where('status', 'payable')->sum('commission_amount'),
            'paid_commissions' => (float) Commission::where('status', 'paid')->sum('commission_amount'),
            'pending_payouts' => (float) Payout::whereIn('status', ['pending', 'approved'])->sum('amount'),
            'net_platform_revenue' => $paidSales - $commissions,
        ];
        ...
        $topPartners = ProgramPartner::with(['user', 'program'])
            ...
            ->orderByDesc('earnings')->limit(8)->get();
        ...
        $businesses = User::role('program_manager')->latest()->limit(10)->get()->map(function ($business) {
            ...
            $business->dashboard_metrics = [
                'programs' => $programIds->count(),
                'partners' => ProgramPartner::whereIn('program_id', $programIds)->count(),
                'orders' => Order::whereIn('program_id', $programIds)->where('status', 'paid')->count(),
        ...
        $pendingActions = [
            ['label' => 'Business approvals', 'count' => $stats['pending_businesses'], 'route' => 'admin.businesses.index', 'tone' => 'amber'],
            ['label' => 'Partner approvals', 'count' => $stats['pending_partners'], 'route' => 'admin.partners.index', 'tone' => 'violet'],
            ['label' => 'Partner payouts', 'count' => Payout::whereIn('status', ['requested', 'pending'])->count(), 'route' => 'admin.payouts.index', 'tone' => 'blue'],
            ['label' => 'Business payouts', 'count' => DB::table('business_payouts')->whereIn('status', ['requested', 'pending'])->count(), 'route' => 'admin.business-payouts.index', 'tone' => 'emerald'],
        ];
